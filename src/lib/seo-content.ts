...
> = {
  'fruehstueck-brunch-in-gd': {
    ...
    listSlugs: ['top-10-fruehstueck-brunch-in-aalen', 'top-10-cafes-in-aalen'],
  ...
  'fitnessstudio-finden-gmuend': {
    ...
    listSlugs: ['top-10-fitnessstudios-in-aalen', 'top-10-sportangebote-in-aalen'],
  ...
  'guten-zahnarzt-in-gd-waehlen': {
    ...
    listSlugs: ['top-10-zahnarzte-in-aalen', 'top-10-zahnreinigung-und-prophylaxe-in-aalen'],
  ...
  'friseur-in-gd-finden': {
    ...
    listSlugs: ['top-10-friseure-in-aalen', 'top-10-barbiere-in-aalen'],
  ...
  'werbeagentur-in-gd-finden': {
    ...
    listSlugs: ['top-10-werbeagenturen-in-aalen', 'top-10-digitalagenturen-in-aalen'],
    companySlugs: ['achtender', 'rawimedia', 'eberle-werbeagentur'],
  },
  'it-dienstleister-gd-vergleichen': {
    categorySlug: 'dienstleister',
    subcategorySlug: 'it-dienstleister',
    listSlugs: ['top-10-it-dienstleister-in-aalen', 'top-10-digitalisierungsberatung-in-aalen'],
...
export function getListSeoBlocks(list: Top10List, category?: Category) {
  ...
  return {
    intro:
      list.seoIntro ||
      `${list.title} ordnet ein lokales Themenfeld für ${category?.label || 'Aalen'} redaktionell ein und verbindet konkrete Adressen mit dem typischen Suchanlass dahinter.`,
    summary:
      categoryLead ||
    ...
    idealFor: [
      `Suchende, die ${list.title.replace('Top 10 ', '').toLowerCase()} in ${category?.label || 'Aalen'} schneller vergleichen wollen`,
    ...
    faq:
      ...
      ([
        ...
        },
        {
          ...
          answer:
            'Vor allem für Menschen, die in Aalen gezielt vergleichen wollen und neben Namen auch Profil, Anlass und thematische Passung sehen möchten.',
        },
        {
          question: 'Kann sich die Reihenfolge ändern?',
          answer:
            'Ja. AA Listen versteht Listen als redaktionell überprüfbare Arbeitsstände. Neue Informationen, Korrekturen oder bessere Profilklarheit können spätere Anpassungen sinnvoll machen.',
        },
      ] satisfies FAQItem[]),
  }
}
...
export function getCategorySeoBlocks(category: Category) {
  ...
  return {
    intro:
      ...
      `${category.label} in Aalen profitieren von klarer Einordnung, damit aus einer bloßen Übersicht ein nutzbarer lokaler Themen-Hub wird.`,
    ...
    faq: [
      {
        ...
        answer:
          'Die Kategorieseite bündelt veröffentlichte Themenfelder, passende Listen und lokale Profile zu einem gemeinsamen Einstiegspunkt für Suchende in Aalen.',
      },
      {
        question: 'Warum sind nicht alle Unterkategorien sofort vollständig ausgebaut?',
        answer:
          'AA Listen baut Inhalte bewusst clusterweise aus. Themen ohne ausreichenden redaktionellen Kontext bleiben zwar navigierbar, werden aber bis zur vollständigen Ausarbeitung nicht als Suchziel forciert.',
      },
      {
...
export function getSubcategorySeoBlocks(category: Category, subcategory: Subcategory, hasList: boolean) {
  ...
  return {
    intro: hasList
      ? `${title} werden auf AA Listen nicht nur gesammelt, sondern lokal eingeordnet. So entsteht aus einer bloßen Branchenübersicht eine Suchhilfe, die schneller zu passenden Anbietern und relevanten Listen führt.`
    ...
    guidance: hasList
      ? `Nutzer suchen in diesem Thema meist nicht einfach nach irgendeinem Treffer, sondern nach einer Auswahl, die zum konkreten Anlass in Aalen passt. Genau darauf ist diese Seite ausgerichtet.`
    ...
    faq: [
      {
        ...
        answer: hasList
          ? `Die Seite bündelt die wichtigste lokale Einstiegsperspektive für ${subcategory.label} in Aalen und verknüpft die redaktionelle Hauptliste mit weiteren thematisch passenden Inhalten.`
      ...
      },
      {
        ...
        answer:
          'AA Listen baut Kategorien in Clustern aus: mit weiteren Listen, Blogartikeln, internen Links und zusätzlichen Unternehmensprofilen, sobald genug lokaler Kontext vorhanden ist.',
      },
    ] satisfies FAQItem[],
  }
}
...
export function getCompanySeoBlocks(company: Company, featuredLists: Top10List[]) {
  ...
  return {
    summary: `${company.name} ist in AA Listen als lokale Adresse in Aalen eingeordnet. Ziel ist nicht nur ein Name im Verzeichnis, sondern ein verständliches Profil für typische Suchanlässe, Leistungen und thematische Cluster.`,
    specializations:
    ...
    faq: (
      company.faq && company.faq.length > 0
        ...
        : [
            {
              question: `Wofür ist ${company.name} auf AA Listen besonders relevant?`,
              answer: `${company.name} wird vor allem über seine sichtbaren Leistungen, den lokalen Bezug und die thematische Passung innerhalb von Aalen eingeordnet.`,
            },
            {
